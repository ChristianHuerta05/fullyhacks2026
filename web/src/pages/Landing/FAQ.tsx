    ...
    <div className="min-h-screen flex flex-col items-center justify-center text-slate-100 w-full relative mt-5 gap-12">
      ...
      <div className= "flex flex-col gap-6 w-full max-w-400 px-4">
        {faqList.map((faqListItem, index) => (
          <FAQDropdown question={faqListItem.question} answer={faqListItem.answer} idx={index} openedFAQ={openedFAQ} setOpenedFAQ={setOpenedFAQ}
          />
        ))}
      </div>
    </div>
...
function FAQDropdown({
  ...
  return (
    <button
      onClick={handleClick}
      className="flex justify-between items-start gap-4 py-4 px-5 sm:px-8 w-full rounded-2xl sm:rounded-[30px] border-2 sm:border-[5px] border-white text-left ursor-pointer hover:bg-white/10 transition-colors duration-200"
    >
      <div className="flex flex-col max-w-[calc(100%-40px)]">
        ...
        {openedFAQ === idx && (
          <p className="text-base sm:text-xl md:text-4xl font-bagel wrap-break-word text-left">{answer}</p>
        )}
      </div>
